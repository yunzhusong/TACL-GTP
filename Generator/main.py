...
def main():
    ...
    # find the best model checkpoint for prediction
    #if training_args.find_best_checkpoint:
    #    model_args.model_name_or_path = _find_best_checkpoint(model_args.model_name_or_path)
    ...
    #bart_local_path = "../results/pens_ghg_own/bart/checkpoint-98000"
    ...
    if training_args.corrupt:
        tokenizer.model_input_names.append("special_tokens_mask")
    ...
    # If add userize dot product loss
    if training_args.userize_dot:

        from recommender.recommender import BAREC
        from config.arguments_rec import RecSeq2SeqTrainingArguments as rec_args
        ckpt = "../results/pens_rec_own/bart_v4_vae_eval_gen/REC/checkpoint-47500/pytorch_model.bin"
        rec_base = AutoModelForSeq2SeqLM.from_pretrained("facebook/bart-base")
        recommender = BAREC(rec_base, rec_args)
        recommender.load_state_dict(torch.load(ckpt))
    ...
    if training_args.do_predict:
        ...
        training_args.text_file="../results_important/pens_ghg_own/bart/checkpoint-98000/all_news/generated_predictions.txt"

        _, _, test_dataset = build_datasets(data_args, training_args, model_args, tokenizer)
        ...
        if training_args.find_best_checkpoint:
            ...
            """
            model_args.model_name_or_path = _find_best_checkpoint(output_dir, "max")
            training_args.output_dir = model_args.model_name_or_path
            model.load_state_dict(torch.load(model_args.model_name_or_path+"/pytorch_model.bin"), strict=False)
            trainer = build_trainer(model_args, data_args, training_args, model, tokenizer, None, None)
            logger.warning(f"Load model from {model_args.model_name_or_path}")
            ...
            model_args.model_name_or_path = _find_best_checkpoint(output_dir, "min")
            training_args.output_dir = model_args.model_name_or_path
            model.load_state_dict(torch.load(model_args.model_name_or_path+"/pytorch_model.bin"), strict=False)
            trainer = build_trainer(model_args, data_args, training_args, model, tokenizer, None, None)
            logger.warning(f"Load model from {model_args.model_name_or_path}")
