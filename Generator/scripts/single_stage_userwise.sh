...
model_name_or_path="bart_userize_average_v4/checkpoint-13000"
...
for ((userwise_index=7; userwise_index<=7; userwise_index++));
do
...
# ===== END ==== #
for ((userwise_index=17; userwise_index<=17; userwise_index++));
do
	exp_name="$prefix/index$userwise_index"
	#exp_name="dev_3"
	mkdir -p $out_dir/$model_name_or_path/$exp_name
